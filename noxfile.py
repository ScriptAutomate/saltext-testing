# pylint: disable=missing-module-docstring,import-error,protected-access,missing-function-docstring,unspecified-encoding,consider-using-f-string
import datetime
import json
import os
import pathlib
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/saltext_testing/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/saltext_testing/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/saltext_testing/*"
            )
            ...
            #    "--include=src/saltext/saltext_testing/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/saltext_testing/config/schemas",
    )
